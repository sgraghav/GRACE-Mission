...
\end{equation}
 ...
 so, 
 \begin{equation}
 \widetilde{X}= \Bigg( {Q_{\tilde{x}\tilde{x}}}^{-1} + \frac{\lambda^T}{2}{Q_{xx}^{-1}}^{-1} \Bigg) {Q_{\tilde{x}     \tilde{x}}}^{-1}  \tilde{x} 
 \end{equation}
...
For instance computing SNR per coefficient will cause trouble if $\epsilon$ = 0.
